...
func TestLoadExamplesConfig(t *testing.T) {
	...
	t.Run("missing file", func(t *testing.T) {
		t.Parallel()
		cfg, err := config.LoadExamplesConfig(t.TempDir())
		if err != nil || cfg != nil {
	...
	t.Run("malformed file", func(t *testing.T) {
		...
		if _, err := config.LoadExamplesConfig(dir); err == nil {
...
`
		...
		cfg, err := config.LoadExamplesConfig(dir)
		if err != nil {
			t.Fatal(err)
		}
		want := &config.ExamplesConfig{
			Packages:      []string{"age"},
			Env:           map[string]string{"TZ": "UTC"},
			Fixtures:      []config.Fixture{{Path: "notes.md", Contents: "hello"}},
			Substitutions: map[string]string{"<key>": "dummy"},
			Steps: []config.StepRule{
